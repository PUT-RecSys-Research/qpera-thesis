...
class KnowledgeEmbedding(nn.Module):
    ...
    def compute_loss(self, batch_triples):
        ...
        for rel_idx_tensor in unique_rel_idxs:
            ...
            if relation_name not in self.relation_info:
                print(f"Warning: Relation '{relation_name}' (from index {rel_idx_int}) not found in model's self.relation_info. Skipping loss calc.")
                continue
    ...
    def neg_loss(self, entity_head_name, relation_name, entity_tail_name, entity_head_idxs, entity_tail_idxs):
