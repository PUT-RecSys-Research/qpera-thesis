...
from .rl_utils import (
    ITEMID, KG_RELATION, PATH_PATTERN, RATED, SELF_LOOP, 
    TITLE, USERID, WATCHED, load_embed, load_kg
)
...
class BatchKGEnvironment:
    ...
    def _compute_user_product_scales(self) -> None:
        ...
        u_p_scores = np.dot(
            self.embeds[USERID] + self.embeds[WATCHED][0], 
            self.embeds[ITEMID].T
        )
    ...
    def _get_candidate_actions(
        ...
        for r in relations_nodes:
            ...
            next_node_ids = [
                n for n in next_node_ids 
                if (next_node_type, n) not in visited_nodes
            ]
            candidate_acts.extend(zip([r] * len(next_node_ids), next_node_ids))
        
        return candidate_acts

    def _trim_actions_by_score(
        self, 
        path: List[Tuple], 
        candidate_acts: List[Tuple]
    ) -> List[Tuple]:
        ...
        candidate_acts = sorted(
            [candidate_acts[i] for i in candidate_idxs], 
    ...
    def _get_state(self, path: List[Tuple]) -> np.ndarray:
        ...
        if len(path) == 2:
            ...
            return self.state_gen(
                user_embed, curr_node_embed, last_node_embed, 
                last_relation_embed, zero_embed, zero_embed
            )
        ...
        return self.state_gen(
            user_embed, curr_node_embed, last_node_embed, last_relation_embed,
            older_node_embed, older_relation_embed
        )
