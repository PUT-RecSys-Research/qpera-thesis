...
class KaggleDatasetDownloader:
    ...
    def _check_kaggle_auth(self) -> None:
        ...
        try:
            subprocess.run(["kaggle", "datasets", "list", "--max-size", "1"], capture_output=True, check=True, timeout=30)
    ...
    def _download_from_kaggle(self, dataset_name: str, force: bool = False) -> None:
        ...
        try:
            ...
            cmd = ["kaggle", "datasets", "download", config["kaggle_dataset"], "--path", str(dataset_dir), "--unzip"]
            ...
            raise RuntimeError(f"Download timeout for {dataset_name} (>10 minutes)\nPlease check your internet connection")
    ...
    def _process_movielens(self, dataset_dir: Path) -> None:
        ...
        unnecessary_files = ["genome_scores.csv", "genome_tags.csv", "README.txt", "ml-20m", "link.csv"]
    ...
    def _process_amazonsales(self, dataset_dir: Path) -> None:
        ...
        for file_path in all_files:
            if file_path.name not in ["amazon.csv", ".gitkeep"] and file_path.is_file():
                file_path.unlink()
    ...
    def download_dataset(self, dataset_name: str, force: bool = False) -> bool:
        ...
        if dataset_name not in self.DATASET_CONFIG:
            raise ValueError(f"Unknown dataset: {dataset_name}. Available: {list(self.DATASET_CONFIG.keys())}")
...
def main():
    ...
    parser.add_argument("--setup-help", action="store_true", help="Show Kaggle setup instructions")
