...
def _generate_sentiment_ratings(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["sentiment_score"] = df["combined_review"].apply(lambda text: sid.polarity_scores(text)["compound"])
...
def generate_amazon_ratings(input_file: str, output_file: str, seed: int = 42, date_range: Optional[tuple] = None) -> pd.DataFrame:
    """
