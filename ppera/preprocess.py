...
def preprocess_rl(dataset, want_col, num_rows, ratio, seed, force_reprocess=True):
    ...
    try:
        ...
        #                         seed=args.seed)
        data_df = load_dataframe(dataset_name=dataset.lower(), # loader uses lowercase names
                                want_col=want_col,
                                num_rows=num_rows,
                                seed=seed)
    except KeyError as e:
    ...
    if not force_reprocess and os.path.exists(processed_dataset_file):
        ...
        try:
            ...
            if not isinstance(processed_dataset, dict) or 'entity_maps' not in processed_dataset or 'relations' not in processed_dataset:
                 ...
                 force_reprocess = True # Force reprocessing if cache is bad
        except Exception as e:
            print(f"Failed to load cached processed dataset ({e}). Reprocessing...")
            processed_dataset = None # Invalidate cache
            force_reprocess = True # Force reprocessing if loading fails
    ...
    try:
        train_df, test_df = python_stratified_split(
            data_df,
            ratio=ratio,      # Corresponds to train set size (e.g., 0.75 for 75% train)
    ...
    if force_reprocess or not os.path.exists(kg_file):
        ...
        try:
            ...
            save_kg(dataset, kg) # Saves to kg.pkl
        except Exception as e:
