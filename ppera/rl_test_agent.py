...
def run_evaluation(path_file, train_labels, test_labels, TOP_K, data, train, test, args):
    ...
    if args.dataset == AMAZONSALES:
        ...
        try:
            ...
            if not user_inv_map or not item_inv_map:
                ...
                if not top_k.empty:
                    print(f"DEBUG: Sample of 'top_k' after mapping IDs:\n{top_k.head().to_string()}")
            ...
            if not user_inv_map or not item_inv_map:
                ...
                if not top.empty and "userID" in top.columns:
                    ...
                    top.dropna(
                        subset=[
                            "userID",
                            "itemID" if "itemID" in top.columns and item_inv_map else "userID",
                        ],
                        inplace=True,
                    )  # Adjust dropna
                ...
                if not top_k.empty and "userID" in top_k.columns:
                    ...
                    top_k.dropna(
                        subset=[
                            "userID",
                            "itemID" if "itemID" in top_k.columns and item_inv_map else "userID",
                        ],
                        inplace=True,
                    )
    ...
    if train is not None and not train.empty:
        dfs_to_standardize_list.append(train)
    if test is not None and not test.empty:
        dfs_to_standardize_list.append(test)
    if "top" in locals() and top is not None and not top.empty:
        dfs_to_standardize_list.append(top)
    if "top_k" in locals() and top_k is not None and not top_k.empty:
        dfs_to_standardize_list.append(top_k)
                ...
                if target_type is str:
                    ...
                    if not pd.api.types.is_integer_dtype(current_dtype):
                        ...
                        try:
                            ...
                            print(
                                f"Sample offending values in {col_name}: "
                                f"{df_item[pd.to_numeric(df_item[col_name], errors='coerce').isna() & df_item[col_name].notna()][col_name].unique()[:5]}"
                            )
    ...
    if not test.empty and "userID" in test.columns:
        ...
        if not user_map and args.dataset in [
            AMAZONSALES,
            POSTRECOMMENDATIONS,
        ]:  # user_map should have been loaded for AMAZONSALES
        ...
        if problematic_user_indices_for_pred_labels:
            print(
                f"DEBUG (pred_labels check): {len(problematic_user_indices_for_pred_labels)} users from test set have no items in 'pred_labels'. "
                f"Sample indices: {list(problematic_user_indices_for_pred_labels)[:5]}"
            )
    ...
    if not test.empty and "userID" in test.columns:
        ...
        if not top_k.empty and "userID" in top_k.columns:
            ...
            if users_missing_from_top_k_raw:
                print(
                    f"  DEBUG (top_k raw): {len(users_missing_from_top_k_raw)} test userIDs missing from 'top_k' (after mapping, before filtering)."
                    f" Sample: {list(users_missing_from_top_k_raw)[:5]}"
                )
        else:
            print("  DEBUG (top_k raw): 'top_k' is empty or has no 'userID' column.")
            if not test.empty:
                users_missing_from_top_k_raw = test_user_ids_original  # All are missing
        ...
        if not top_k_filtered.empty and "userID" in top_k_filtered.columns:
            ...
            if known_missing_user_ids_set:
                print(
                    f"  DEBUG: {len(known_missing_user_ids_set)} test uIDs missing from 'top_k_filtered' after filter."
                    f"Sample: {list(known_missing_user_ids_set)[:5]}"
                )
                ...
                if users_removed_by_filtering:
                    print(f"{len(users_removed_by_filtering)} users lost all preds due to train filter. Sample: {list(users_removed_by_filtering)[:3]}")

        else:
            print("  DEBUG (top_k_filtered): 'top_k_filtered' is empty or has no 'userID' column.")
            if not test.empty:
                known_missing_user_ids_set = test_user_ids_original  # All are missing
    ...
    if all(col in test.columns for col in id_cols_test):
        ...
        if all(col in top_k_filtered.columns for col in id_cols_pred) and not top_k_filtered.empty:
            ...
            if not missing_test_interactions_df.empty:
                print(f"WARNING: {len(missing_test_interactions_df)} (userID, itemID) pairs from 'test' not found in 'top_k_filtered'.")
                ...
                print(f"Test samples outside 'top_k_filtered':\n{missing_test_interactions_df[id_cols_test + ['rating']].head().to_string()}")
            elif not top_k_filtered.empty:
                print(
                    "INFO: All (userID, itemID) interactions from 'test' seem to have a corresponding (userID, itemID) in 'top_k_filtered'."
                    "(This doesn't mean ratings/predictions match, just that the pairs exist)."
                )
        else:
...
def test(
    ...
    if args.run_eval:
        ...
        )  # human_recs_top_k is a dict and dont have atribute head - i have to provide here a single user top_k
...
def test_agent_rl(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.99,
        help="reward discount factor (used in model definition).",
    )
