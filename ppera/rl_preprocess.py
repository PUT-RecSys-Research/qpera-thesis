...
def _create_entity_mappings(df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
    ...
    for entity_name, col_name in entity_columns.items():
        ...
        entity_maps[entity_name] = _create_single_entity_mapping(
            df, entity_name, col_name
        )
        
    return entity_maps


def _create_single_entity_mapping(
    df: pd.DataFrame, 
    entity_name: str, 
    col_name: str
) -> Dict[str, Any]:
    ...
    if entity_name == GENRES:
        ...
        all_values = set(
            g for genres_list in valid_series.astype(str) 
            for g in genres_list.split() if g
        )
    else:
...
def _get_entity_index(entity_type: str, value: Any, entity_maps: Dict[str, Dict[str, Any]]) -> Optional[int]:
    ...
    if (entity_type in entity_maps and 
        pd.notna(value) and 
        value in entity_maps[entity_type]["map"]):
        return entity_maps[entity_type]["map"][value]
    return None


def _extract_watched_relations(
    df: pd.DataFrame, 
    entity_maps: Dict[str, Dict[str, Any]]
) -> List[Tuple[int, int]]:
...
def _extract_belong_to_relations(
    df: pd.DataFrame, 
    entity_maps: Dict[str, Dict[str, Any]]
) -> List[Tuple[int, int]]:
...
def _extract_described_as_relations(
    df: pd.DataFrame, 
    entity_maps: Dict[str, Dict[str, Any]]
) -> List[Tuple[int, int]]:
...
def _extract_rated_relations(
    df: pd.DataFrame, 
    entity_maps: Dict[str, Dict[str, Any]], 
    watched_relations: List[Tuple[int, int]]
) -> List[Tuple[int, int]]:
    """Extract RATED relations (USERID -> TITLE) derived from watched relations."""
    if (USERID not in entity_maps or 
        TITLE not in entity_maps or 
        ITEMID not in entity_maps):
        return []
...
def _extract_user_rated_value_relations(
    df: pd.DataFrame, 
    entity_maps: Dict[str, Dict[str, Any]]
) -> List[Tuple[int, int]]:
    """Extract USER_RATED_WITH_VALUE relations (USERID -> RATING)."""
    if (USERID not in entity_maps or 
        RATING not in entity_maps or 
        "userID" not in df.columns or 
...
def _extract_rating_item_relations(
    df: pd.DataFrame, 
    entity_maps: Dict[str, Dict[str, Any]]
) -> List[Tuple[int, int]]:
    """Extract RATING_VALUE_FOR_ITEM relations (RATING -> ITEMID)."""
    if (RATING not in entity_maps or 
        ITEMID not in entity_maps or 
        "rating" not in df.columns or 
        "itemID" not in df.columns):
        return []
...
def _calculate_distributions(processed_data: Dict[str, Any]) -> Dict[str, np.ndarray]:
    ...
    for relation_name, relation_data in relations.items():
        ...
        try:
            distribution = _calculate_single_distribution(
                relation_name, relation_data, entity_maps
            )
...
def _calculate_single_distribution(
    relation_name: str, 
    relation_data: List[Tuple[int, int]], 
    entity_maps: Dict[str, Dict[str, Any]]
) -> Optional[np.ndarray]:
...
def _is_valid_interaction(
    ...
    return (pd.notna(uid_orig) and 
            pd.notna(iid_orig) and 
            pd.notna(rating_val) and 
            uid_orig in user_map and 
            iid_orig in item_map)
...
def preprocess_rl(
    ...
    # Apply modifications
    if privacy:
        data_df = _apply_privacy_modifications(
            data_df, hide_type, columns_to_hide, fraction_to_hide, records_to_hide, seed
        )

    if personalization:
        train_df = _apply_personalization_modifications(
            data_df, train_df, fraction_to_change, change_rating, seed
        )
...
def _split_data(
    ...
    try:
        train_df, test_df = python_stratified_split(
            data_df, ratio=ratio, col_user="userID", col_item="itemID", seed=seed
        )
        print("Stratified split successful.")
    except Exception as e:
        print(f"Stratified split failed ({e}), using simple random split.")
        train_df, test_df = train_test_split(
            data_df, test_size=1.0 - ratio, random_state=seed
        )
...
def _get_or_create_processed_dataset(
    dataset: str, 
    data_df: pd.DataFrame, 
...
def _is_valid_processed_dataset(processed_dataset: Any) -> bool:
    ...
    return (isinstance(processed_dataset, dict) and 
            "entity_maps" in processed_dataset and 
            "relations" in processed_dataset)
