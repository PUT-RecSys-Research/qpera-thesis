...
def calculate_predictions(
    all_users_top_k_full_candidates: Dict[int, List[Tuple[float, float, int]]], 
    median_kge_score: Optional[float], 
    scale_factor_kge: Optional[float], 
    top_k: int
    ...
    for user_id, candidates in all_users_top_k_full_candidates.items():
        ...
        top_1_prediction = _process_top_1_prediction(
            user_id, candidates[0], median_kge_score, scale_factor_kge
        )
        if top_1_prediction:
            top_1_data.append(top_1_prediction)
        
        # Process top-k predictions
        user_top_k_predictions = _process_top_k_predictions(
            user_id, candidates[:top_k], median_kge_score, scale_factor_kge
        )
...
def _process_top_1_prediction(
    user_id: int,
    top_candidate: Tuple[float, float, int],
    median_kge_score: float,
    scale_factor_kge: float
...
def _calculate_sigmoid_prediction(
    kge_score: float, 
    median_kge_score: float, 
    scale_factor_kge: float
) -> float:
...
def get_recommended_normalization_params(
    all_users_top_k_full_candidates: Dict[int, List[Tuple[float, float, int]]]
    ...
    print(f"Recommended normalization parameters:")
...
def validate_predictions(df_predictions: pd.DataFrame) -> bool:
    ...
    if predictions.min() < 0 or predictions.max() > 5:
        print(f"Warning: Predictions outside expected range [0, 5]. "
              f"Actual range: [{predictions.min():.4f}, {predictions.max():.4f}]")
        return False
...
def calculate_predictons(*args, **kwargs):
    ...
    """
    print("Warning: Using legacy function name 'calculate_predictons'. "
          "Consider using 'calculate_predictions' instead.")
