...
class ExperimentRunner:
    ...
    def _build_configurations(self) -> List[Dict[str, Any]]:
        ...
        clear_config.update({"run_label": "Clean", "privacy": False, "personalization": False})
    ...
    def _run_single_experiment(self, experiment: Dict[str, Any], config: Dict[str, Any], progress_info: Dict[str, int]) -> bool:
        """
        ...
        self.logger.info(f"Starting Individual Experiment ({progress_info['current_exp']}/{progress_info['total_individual']}): {exp_progress_label}")
        ...
        func_args.update({"dataset": dataset, "num_rows": rows})
    ...
    def _run_configuration(self, config: Dict[str, Any], config_idx: int, total_configs: int, current_exp_count: int) -> tuple[int, int, List[str]]:
        """
        ...
        """
        run_label = config["run_label"]
        self.logger.info(f"\n{'-' * 20} Starting Configuration {config_idx + 1}/{total_configs}: {run_label} {'-' * 20}")
    ...
    def run_all_experiments(self) -> None:
        ...
        print("  - MLflow server should be running at: http://127.0.0.1:8080")

        for config_idx, config in enumerate(self.configurations):
            config_successful, current_exp_count, config_failed = self._run_configuration(config, config_idx, total_configs, current_exp_count)

            total_successful += config_successful
            all_failed_experiments.extend([f"{config['run_label']}: {failure}" for failure in config_failed])

        # Print final summary
        self._print_final_summary(overall_start_time, total_successful, total_individual, all_failed_experiments)

    def _print_final_summary(self, start_time: float, total_successful: int, total_individual: int, failed_experiments: List[str]) -> None:
